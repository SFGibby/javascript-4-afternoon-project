...
Movie.prototype.changeRating = function(newRate = 5) {
  return (10 % newRate)
}
  ...
  // const user1 = new User['Samuel', 23, 'samgibson124@gmail.com', [{id: 8, title: 'senior', rating: 3}, ]];
  ...
  User.prototype.removeSavedPost = function(lemons){
    
    for(let i = 0; i < this.savedPosts.length; i++)
    if (this.savedPosts[i].id === lemons){
      this.savedPosts.splice(i, 1)
    }
  }

  //for(let i = 0; i< arraypostnum.length - 1; i++)
  // (if arraypostnum[i] === 3){
    //console.log("deleted")
  //}
  //
// for (post in savedPosts) {
  ...
  //   for(let i = 0; i < this.savedPosts.length; i++)
  //   if (this.changePostRating[id] === id) {
  //     this.savedPosts[i].rating === id;
  //   }  
  // }